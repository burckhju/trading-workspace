...
from app.features.market.domain.errors import ConcurrentModification, DomainRuleViolation
...
def translate_market_error(error: Exception) -> ApplicationError:
    if isinstance(error, UnderlyingDeleteReferenced):
        details = tuple(
            ErrorDetail(
                ...
                context={"reference_type": item.reference_type, "object_id": str(item.object_id)},
            )
    ...
    if isinstance(error, (UnderlyingNotFound, ListingNotFound)):
        return ApplicationError(
            code=error.code,
            message=str(error),
