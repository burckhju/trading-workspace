...
from app.features.market_data.api.dependencies import get_daily_price_import_service, get_provider_mapping_service
from app.core.di import ApplicationContainer, get_container
from app.features.market_data.api.dtos import (ImportDailyPricesRequest, ImportDailyPricesResponse, ProviderMappingUpsertRequest, ProviderMappingStateRequest, ProviderMappingResponse, ProviderStatusResponse)
...
async def upsert_provider_mapping(
    ...
    try:
        value = await service.create_or_update(MappingCommand(
            workspace_id=WORKSPACE_ID, listing_id=body.listing_id, provider=body.provider,
            provider_symbol=body.provider_symbol, provider_exchange_code=body.provider_exchange_code,
            actor_id=body.actor_id, actor_name=body.actor_name,
        ))
    except MarketDataError as error:
        raise translate_market_data_error(error) from error
...
async def validate_provider_mapping(
    mapping_id: UUID, body: ProviderMappingStateRequest,
    service: Annotated[ProviderMappingAdministrationService, Depends(get_provider_mapping_service)],
) -> ProviderMappingResponse:
    """Validate and activate one mapping through an explicit administrative action."""
    try:
        value = await service.validate(WORKSPACE_ID, mapping_id, actor_id=body.actor_id, actor_name=body.actor_name)
    except MarketDataError as error:
        raise translate_market_data_error(error) from error
    return ProviderMappingResponse.from_domain(value)
...
async def set_provider_mapping_state(
    ...
    try:
        value = await service.set_enabled(WORKSPACE_ID, mapping_id, enabled=body.enabled, actor_id=body.actor_id, actor_name=body.actor_name)
    except MarketDataError as error:
        raise translate_market_data_error(error) from error
    return ProviderMappingResponse.from_domain(value)
