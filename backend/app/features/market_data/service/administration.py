        ...
        async with self._uow:
            ...
            if existing is None:
                value = ProviderInstrumentMapping(
                    id=self._id_factory(), workspace_id=command.workspace_id,
                    listing_id=command.listing_id, provider=command.provider,
                    provider_symbol=command.provider_symbol,
                    provider_exchange_code=command.provider_exchange_code,
                    status=MappingStatus.DISABLED, validated_at=None,
                ...
                await self._audit(value, command, ChangeType.UPDATED, before.version, value.version, self._diff(before, value))
            await self._uow.mappings.flush()
            await self._uow.commit()
            return value

    async def validate(self, workspace_id: UUID, mapping_id: UUID, *, actor_id: str | None, actor_name: str) -> ProviderInstrumentMapping:
        ...
        async with self._uow:
            ...
            if validation is not None and validation.status is not MappingStatus.ACTIVE:
                ...
                command = MappingCommand(workspace_id, model.listing_id, model.provider, model.provider_symbol, model.provider_exchange_code, actor_id, actor_name)
                await self._audit(value, command, ChangeType.UPDATED, before.version, value.version, self._diff(before, value))
                await self._uow.mappings.flush(); await self._uow.commit()
                return value
            model.status = MappingStatus.ACTIVE
            model.validated_at = now
            model.validation_message = validation.message if validation is not None else "Validated by administrative review"
            model.updated_at = now
            model.version += 1
            value = mapping_to_domain(model)
            command = MappingCommand(workspace_id, model.listing_id, model.provider, model.provider_symbol, model.provider_exchange_code, actor_id, actor_name)
            await self._audit(value, command, ChangeType.ACTIVATED, before.version, value.version, self._diff(before, value))
            await self._uow.mappings.flush(); await self._uow.commit()
            return value

    async def set_enabled(self, workspace_id: UUID, mapping_id: UUID, *, enabled: bool, actor_id: str | None, actor_name: str) -> ProviderInstrumentMapping:
        """Activate a validated mapping or disable it without deleting history."""
        async with self._uow:
            model = await self._uow.mappings.get(workspace_id, mapping_id)
            if model is None:
                raise MarketDataNotFoundError("Provider mapping not found")
            before = mapping_to_domain(model); now = self._now()
            if enabled:
                model.status = MappingStatus.ACTIVE
            ...
            model.updated_at = now; model.version += 1
            value = mapping_to_domain(model)
            command = MappingCommand(workspace_id, model.listing_id, model.provider, model.provider_symbol, model.provider_exchange_code, actor_id, actor_name)
            await self._audit(value, command, change, before.version, value.version, self._diff(before, value))
            await self._uow.mappings.flush(); await self._uow.commit()
            return value

    async def _audit(self, value: ProviderInstrumentMapping, command: MappingCommand, change: ChangeType, before: int | None, after: int, fields: dict) -> None:
        await self._uow.audit_events.append(AuditEventModel(
            id=self._id_factory(), workspace_id=value.workspace_id,
            aggregate_type=AggregateType.PROVIDER_MAPPING, aggregate_id=value.id,
            occurred_at=value.updated_at, actor_type=ActorType.SYSTEM_USER,
            actor_id=command.actor_id, actor_display_name=command.actor_name,
            data_origin=DataOrigin.MANUAL, change_type=change,
            version_before=before, version_after=after, field_changes=fields,
        ))

    @staticmethod
    def _diff(before: ProviderInstrumentMapping, after: ProviderInstrumentMapping) -> dict:
        ignored = {"created_at", "updated_at", "version"}; result = {}
        for key, old in asdict(before).items():
            if key in ignored: continue
            new = getattr(after, key)
            if old != new: result[key] = {"before": str(old) if old is not None else None, "after": str(new) if new is not None else None}
        return result
