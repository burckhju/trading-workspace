...
from app.features.market_data.persistence.repositories import DailyPriceRepository, ProviderInstrumentMappingRepository, SqlAlchemyDailyPriceRepository, SqlAlchemyProviderInstrumentMappingRepository
from app.features.market.persistence.repositories import AuditEventRepository, SqlAlchemyAuditEventRepository
...
class SqlAlchemyMarketDataUnitOfWork:
    ...
    async def __aexit__(self, exc_type: type[BaseException] | None, exc: BaseException | None, traceback: TracebackType | None) -> None:
        if exc_type is not None: await self.rollback()
