    ...
    try:
        async with database.session_context() as session:
            ...
            for mapping, source_listing, warrant, source_venue in rows:
                ...
                if selections:
                    raise ValueError(
                        f"GETTEX_MAPPING_HAS_ACTIVE_POSITION_SELECTION_{mapping.id}"
                    )
                ...
                if len(target_listings) > 1:
                    raise ValueError(
                        f"GETTEX_TARGET_LISTING_CONFLICT_{warrant.isin}_{target_mic}"
                    )
                ...
                if target_listings:
                    ...
                    if target_listing.lifecycle_status != WarrantLifecycle.ACTIVE:
                        raise ValueError(
                            f"GETTEX_TARGET_LISTING_INACTIVE_{warrant.isin}_{target_mic}"
                        )
                    if (
                        target_listing.quotation_currency_code
                        != source_listing.quotation_currency_code
                    ):
                        raise ValueError(
                ...
                plans.append(
                    {
                        ...
                        "source_listing_id": str(source_listing.id),
                        "source_mic": source_venue.mic,
                        "target_listing_id": str(target_listing.id),
                        "target_mic": target_mic,
