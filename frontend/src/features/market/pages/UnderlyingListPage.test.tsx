...
function renderPage() {
  return render(<MemoryRouter><UnderlyingListPage /></MemoryRouter>);
}

beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(marketApiClient.listTradingVenues).mockResolvedValue({ items: [{
    id: '00000000-0000-4000-8001-000000000001', mic: 'XETR', name: 'Xetra', country_code: 'DE', timezone: 'Europe/Berlin', reference_version: 'FT-001-V1',
  }] });
...
describe('UnderlyingListPage', () => {
  it('loads controlled filters and renders the primary listing without detail requests', async () => {
    ...
    expect(await screen.findByRole('link', { name: 'Siemens AG' })).toHaveAttribute('href', '/underlyings/11111111-1111-4111-8111-111111111111');
    ...
    expect(marketApiClient.searchUnderlyings).toHaveBeenCalledWith(expect.objectContaining({ lifecycleStatus: 'ACTIVE', offset: 0, limit: 25 }), expect.any(AbortSignal));
  });
  ...
  it('submits search and server-side venue and currency filters', async () => {
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: 'Markt' }), '00000000-0000-4000-8001-000000000001');
    await user.selectOptions(screen.getByRole('combobox', { name: 'Währung' }), 'EUR');
    await user.click(screen.getByRole('button', { name: 'Suchen' }));

    await waitFor(() => expect(marketApiClient.searchUnderlyings).toHaveBeenLastCalledWith({
      query: 'SIE', lifecycleStatus: 'ACTIVE', tradingVenueId: '00000000-0000-4000-8001-000000000001', currencyCode: 'EUR', offset: 0, limit: 25,
  ...
  it('shows an empty state for an empty result', async () => {
    ...
    renderPage();
    expect(await screen.findByRole('heading', { name: 'Keine Basiswerte gefunden' })).toBeInTheDocument();
  });
});
