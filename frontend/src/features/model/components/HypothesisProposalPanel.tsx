...
export function HypothesisProposalPanel({ hypothesisId }: HypothesisProposalPanelProps) {
  ...
  const candidateCurrent = isCandidateModel && selectedVersion
    ? readCandidateConfiguration(selectedVersion.definition)
    : null;
  const candidateImpact = candidateCurrent && candidatePolicy
  ...
  const candidateUnsupported = isCandidateModel && selectedVersion !== undefined && candidateCurrent === null;
  ...
  const submitDisabled = saving || !modelId || !baseVersionId ||
  ...
  return (
    <div className="mt-4 rounded-lg border border-slate-800 p-4">
      ...
      {error !== null && <div className="mt-3"><ErrorNotice error={error} /></div>}

      <form className="mt-4 space-y-4" onSubmit={(event) => void submit(event)}>
        <label className="block text-sm">
          <span className="text-slate-400">Governed Model</span>
          <select className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2" value={modelId}
            onChange={(event) => { setModelId(event.target.value); setCandidatePolicy(''); }} required>
            <option value="">Model auswählen</option>
            {models.map((model) => <option key={model.id} value={model.id}>{model.model_key} · {model.name}</option>)}
          </select>
        </label>

        <label className="block text-sm">
          <span className="text-slate-400">APPROVED Base-Version</span>
          <select className="mt-1 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2" value={baseVersionId}
            onChange={(event) => {
              ...
              if (nextVersion) {
                ...
                setCandidatePolicy(readCandidateConfiguration(nextVersion.definition)?.policy ?? '');
              }
            }} required disabled={!modelId || loadingVersions}>
        ...
        {modelId && !loadingVersions && versions.length === 0 && <p className="text-sm text-amber-300">Dieses Model hat keine APPROVED Version.</p>}

        {isCandidateModel ? (
          <div className="space-y-4" aria-live="polite">
            {candidateUnsupported ? (
              <div className="rounded-md border border-amber-700 p-3 text-sm text-amber-200" role="alert">
                Diese Candidate Definition wird vom spezialisierten Editor nicht vollständig verstanden und kann hier nicht verlustfrei geändert werden.
              </div>
              ...
              <>
                <div className="rounded-md border border-slate-700 p-3 text-sm">
                  ...
                  <p className="mt-1">Model-Version {selectedVersion.version} · {candidateCurrent.schema}</p>
                  <p className="mt-1">Candidate market context policy: {candidatePolicyLabel(candidateCurrent.policy)}</p>
                  {candidateCurrent.schema === CANDIDATE_SCHEMA_V1 && (
                    <p className="mt-2 text-slate-400">Legacy 1.0 bleibt unverändert. Eine vorgeschlagene Policy-Änderung erzeugt ausdrücklich eine neue 2.0-Definition.</p>
                  )}
                </div>
                <fieldset className="rounded-md border border-slate-700 p-3">
                  <legend className="px-1 text-sm font-medium">Proposed Candidate Policy · Schema 2.0</legend>
                  <label className="mt-2 flex gap-2 text-sm"><input type="radio" name="candidate-policy" value="FAVORABLE_AND_CAUTIOUS"
                    checked={candidatePolicy === 'FAVORABLE_AND_CAUTIOUS'} onChange={() => setCandidatePolicy('FAVORABLE_AND_CAUTIOUS')} />FAVORABLE + CAUTIOUS</label>
                  <label className="mt-2 flex gap-2 text-sm"><input type="radio" name="candidate-policy" value="FAVORABLE_ONLY"
                    checked={candidatePolicy === 'FAVORABLE_ONLY'} onChange={() => setCandidatePolicy('FAVORABLE_ONLY')} />FAVORABLE only</label>
                </fieldset>
                <div className="rounded-md border border-slate-700 p-3 text-sm">
                  <p className="font-medium">Proposed</p>
                  <p className="mt-1">Candidate market context policy: {candidatePolicy ? candidatePolicyLabel(candidatePolicy) : 'Keine Auswahl'}</p>
                  {candidateNoOp && <p className="mt-2 text-amber-300">Keine Änderung.</p>}
                  {candidateImpact && <><p className="mt-2">{candidateImpact}</p><p className="mt-2 text-xs text-slate-500">Direkte definitorische Auswirkung; keine retrospective Validation, Performance-Prognose oder Empfehlung.</p></>}
                </div>
              </>
            ) : null}
          </div>
          ...
          <label className="block text-sm">
            ...
            <textarea className="mt-1 min-h-48 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-xs"
              value={definition} onChange={(event) => setDefinition(event.target.value)} required />
          </label>
        )}

        {selectedVersion && <p className="text-xs text-slate-500">Ausgangspunkt: Model-Version {selectedVersion.version} · {selectedVersion.id}</p>}
        <label className="block text-sm">
          <span className="text-slate-400">Rationale</span>
          <textarea className="mt-1 min-h-24 w-full rounded-md border border-slate-700 bg-slate-950 px-3 py-2" value={rationale}
            onChange={(event) => setRationale(event.target.value)} required />
        </label>
        <button type="submit" disabled={submitDisabled} className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium disabled:opacity-50">
