  ...
  return (
    <main className="w-full space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">FT-007 · Bestehender TradePlan</p>
          <h1 className="mt-1 text-2xl font-semibold">{detail ? tradePlanReference(detail.plan.id) : 'TradePlan'}</h1>
          {underlying && (
        ...
        <Link to="/trade-plans/overview" className="rounded-lg border border-slate-700 px-4 py-2 text-sm">
          Zur Übersicht
        </Link>
      </header>

      {message && <p role="status" className="rounded-lg border border-slate-700 p-3 text-sm">{message}</p>}
          ...
          <section className="rounded-xl border border-sky-900 bg-sky-950/20 p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-xl font-semibold">Version {current.version} · {current.status}</h2>
              </div>
              <span className="rounded-full border border-sky-800 px-3 py-1 text-xs">{current.direction}</span>
            </div>
            <p className="mt-4 whitespace-pre-wrap text-sm">{current.thesis}</p>
            <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
              <div><dt className="text-slate-500">Entry</dt><dd className="mt-1">{current.entry.price ?? current.entry.trigger ?? current.entry.type}</dd></div>
              <div><dt className="text-slate-500">Stop</dt><dd className="mt-1">{current.invalidation.stop_price ?? '—'}</dd></div>
              <div><dt className="text-slate-500">Target 1</dt><dd className="mt-1">{current.targets[0]?.price ?? '—'}</dd></div>
              <div><dt className="text-slate-500">Nächster Schritt</dt><dd className="mt-1 font-medium">{nextStep(current.status)}</dd></div>
            </dl>
            <div className="mt-5 flex flex-wrap gap-3">
              {current.status === 'DRAFT' && (
                <button disabled={busy} onClick={() => void mutate('submit')} className="rounded-lg border border-sky-700 px-4 py-2 text-sm disabled:opacity-50">
                  Zur Prüfung einreichen
                </button>
              )}
              {current.status === 'READY_FOR_REVIEW' && (
                <button disabled={busy} onClick={() => void mutate('approve')} className="rounded-lg border border-emerald-700 px-4 py-2 text-sm disabled:opacity-50">
                  Explizit freigeben
                </button>
              )}
            ...
            <ol className="mt-4 space-y-2">
              {versions.map((version) => (
                <li key={version.id} className="rounded-lg border border-slate-800 p-3 text-sm">
                  <span className="font-medium">Version {version.version} · {version.status}</span>
                  <span className="ml-2 text-slate-500">{new Date(version.created_at).toLocaleString('de-DE')}</span>
                </li>
              ))}
            </ol>
          </section>
