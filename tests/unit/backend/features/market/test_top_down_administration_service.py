...
async def test_create_and_activate_sector_and_reference():
    ...
    with pytest.raises(ValueError, match="sector code already exists"):
        await service.create_sector(
            workspace_id=workspace, code="tech", name="T", classification_system="X", classification_version="1"
        )

    reference = SimpleNamespace(active=False)
    session.scalar.return_value = reference
    assert (await service.set_market_reference_active(workspace_id=workspace, market_reference_id=uuid4(), active=True)).active
    sector_obj = SimpleNamespace(active=True)
    session.scalar.return_value = sector_obj
    assert not (await service.set_sector_active(workspace_id=workspace, sector_id=uuid4(), active=False)).active

    session.scalar.return_value = None
    with pytest.raises(ValueError, match="market reference not found"):
        await service.set_market_reference_active(workspace_id=workspace, market_reference_id=uuid4(), active=True)
...
async def test_create_sector_reference_and_duplicate_rejected():
    ...
    with pytest.raises(ValueError, match="market reference code already exists"):
        await service.create_sector_reference(
            workspace_id=workspace, code="TECH_IDX", name="x", region="US", reference_version="1"
        )
...
async def test_assignment_creation_paths_and_validation():
    ...
    service._require_reference = AsyncMock(return_value=SimpleNamespace(reference_type=MarketReferenceType.SECTOR_INDEX.value))
    ...
    listing_assignment = await service.assign_reference_listing(
        workspace_id=workspace, market_reference_id=ref_id, listing_id=listing_id,
        valid_from=date(2026, 1, 1), valid_to=None, source=" admin ", source_reference="  ref  ", quality_status="GOOD"
    )
    ...
    benchmark = await service.assign_underlying_benchmark(
        workspace_id=workspace, underlying_id=underlying, market_reference_id=ref_id,
        role=BenchmarkRole.BROAD_MARKET, valid_from=date(2026, 1, 1), valid_to=None,
        source=" admin ", source_reference=" ", quality_status="GOOD"
    )
    ...
    sector_assignment = await service.assign_underlying_sector(
        workspace_id=workspace, underlying_id=underlying, sector_id=sector_id,
        valid_from=date(2026, 1, 1), valid_to=None, source=" admin ", source_reference=None, quality_status="GOOD"
    )
    assert sector_assignment.sector_id == sector_id

    sector_ref = await service.assign_sector_reference(
        workspace_id=workspace, sector_id=sector_id, market_reference_id=ref_id,
        valid_from=date(2026, 1, 1), valid_to=None, source=" admin ", quality_status="GOOD"
    )
    assert sector_ref.market_reference_id == ref_id

    service._require_reference.return_value = SimpleNamespace(reference_type=MarketReferenceType.INDEX.value)
...
async def test_assignment_missing_listing_and_helper_validation():
    ...
    with pytest.raises(ValueError, match="listing not found"):
        await service.assign_reference_listing(
            workspace_id=workspace, market_reference_id=uuid4(), listing_id=uuid4(),
            valid_from=date(2026,1,1), valid_to=None, source="x", source_reference=None, quality_status="GOOD"
        )

    raw = TopDownReferenceAdministrationService(_session())
    with pytest.raises(ValueError, match="valid_to"):
        await raw._ensure_no_overlap(object, workspace, date(2026,2,1), date(2026,1,1), label="x")
    ...
    # SQL construction requires mapped attributes; use a real assignment model indirectly through a patched scalar path.
    ...
    with pytest.raises(ValueError, match="overlapping x"):
        await overlap_service._ensure_no_overlap(
            UnderlyingSectorAssignmentModel, workspace, date(2026,1,1), None,
            UnderlyingSectorAssignmentModel.underlying_id == uuid4(), label="x"
        )
