...
def price(listing_id: UUID, close: str = "101") -> DailyPrice:
    return DailyPrice(
        ...
        close=Decimal(close), adjusted_close=None, volume=None, currency="EUR",
        provider=MarketDataProvider.EODHD, provider_symbol="SAP.XETRA",
        retrieved_at=NOW, source_updated_at=None,
        quality_status=QualityStatus.VALID,
    )


class MappingRepo:
    def __init__(self, mapping: object) -> None: self.mapping = mapping
    async def get(self, workspace_id: UUID, mapping_id: UUID): return self.mapping


class PriceRepo:
    def __init__(self, existing: DailyPriceModel | None = None) -> None:
        self.existing = existing; self.added: list[DailyPriceModel] = []; self.flushed = False
    async def get(self, *args): return self.existing
    async def add(self, value: DailyPriceModel) -> None: self.added.append(value)
    async def flush(self) -> None: self.flushed = True


class Uow:
    def __init__(self, mapping: object, existing: DailyPriceModel | None = None) -> None:
        self.mappings = MappingRepo(mapping); self.daily_prices = PriceRepo(existing)
        self.committed = False; self.rolled_back = False
...
class Provider:
    ...
    async def get_daily_prices(self, request): return self.result


def request(workspace_id: UUID, listing_id: UUID, mapping_id: UUID) -> DailyPriceRequest:
    return DailyPriceRequest(workspace_id, listing_id, mapping_id, date(2026,8,4), date(2026,8,4), uuid4())


def result(value: DailyPrice):
    return MarketDataResult((value,), MarketDataProvider.EODHD,
        MarketDataCapability.HISTORICAL_DAILY_PRICES, uuid4(), NOW,
        CacheStatus.MISS, QualityStatus.VALID, (), 0, 1)
...
async def test_import_inserts_and_commits() -> None:
    ...
    uow = Uow(mapping)
    service = DailyPriceImportService(uow=uow, provider=Provider(result(price(listing_id))), clock=lambda: NOW, id_factory=uuid4)
...
async def test_import_rejects_cross_listing_provider_data_and_rolls_back() -> None:
    ...
    mapping = type("Mapping", (), {"listing_id": listing_id})()
    uow = Uow(mapping)
    service = DailyPriceImportService(uow=uow, provider=Provider(result(price(uuid4()))), clock=lambda: NOW)
