...
from app.features.market_data.service.administration import MappingCommand, ProviderMappingAdministrationService
...
class MappingRepo:
    def __init__(self) -> None:
        self.value = None
        self.flushed = False
    async def find_for_listing(self, *args): return self.value
    ...
    async def add(self, value): self.value = value
    async def list_all(self, workspace_id: UUID, provider=None): return [self.value] if self.value else []
    async def flush(self): self.flushed = True


class AuditRepo:
    def __init__(self) -> None: self.events = []
    async def append(self, value): self.events.append(value)


class Uow:
    def __init__(self) -> None:
        self.mappings = MappingRepo(); self.audit_events = AuditRepo(); self.committed = False
    async def __aenter__(self): return self
    async def __aexit__(self, exc_type: type[BaseException] | None, exc: BaseException | None, tb: TracebackType | None): return None
    async def commit(self): self.committed = True
    async def rollback(self): return None


def command(workspace_id: UUID, listing_id: UUID) -> MappingCommand:
    return MappingCommand(workspace_id, listing_id, MarketDataProvider.EODHD, "sap", "xetra", "admin", "Administrator")


@pytest.mark.asyncio
async def test_create_mapping_is_disabled_and_audited() -> None:
    uow = Uow(); service = ProviderMappingAdministrationService(uow, now=lambda: NOW, id_factory=uuid4)
...
async def test_validate_activates_mapping_and_records_validation_time() -> None:
    ...
    uow.committed = False
    validated = await service.validate(created.workspace_id, created.id, actor_id="admin", actor_name="Administrator")
...
async def test_disable_keeps_mapping_history() -> None:
    uow = Uow(); service = ProviderMappingAdministrationService(uow, now=lambda: NOW, id_factory=uuid4)
    created = await service.create_or_update(command(uuid4(), uuid4()))
    await service.validate(created.workspace_id, created.id, actor_id=None, actor_name="Administrator")
    disabled = await service.set_enabled(created.workspace_id, created.id, enabled=False, actor_id=None, actor_name="Administrator")
...
async def test_technical_validation_failure_keeps_mapping_inactive() -> None:
    uow = Uow(); service = ProviderMappingAdministrationService(
