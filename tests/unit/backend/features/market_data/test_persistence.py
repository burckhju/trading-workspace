...
from app.features.market_data.persistence.mapping import apply_daily_price, daily_price_to_domain, daily_price_to_model, mapping_to_domain, mapping_to_model
from app.features.market_data.persistence.models import DailyPriceModel, ProviderInstrumentMappingModel
from app.features.market_data.persistence.repositories import SqlAlchemyDailyPriceRepository, SqlAlchemyProviderInstrumentMappingRepository
...
def _price(close: str = "101") -> DailyPrice:
    return DailyPrice(listing_id=uuid4(), trading_date=date(2026, 8, 4), open=Decimal("100"), high=Decimal("102"), low=Decimal("99"), close=Decimal(close), adjusted_close=None, volume=None, currency="EUR", provider=MarketDataProvider.EODHD, provider_symbol="SIE.XETRA", retrieved_at=NOW, source_updated_at=None, quality_status=QualityStatus.VALID, warnings=("checked",))
...
def test_models_register_constraints_and_delete_rules() -> None:
    ...
    assert {c.name for c in mapping.constraints if isinstance(c, UniqueConstraint)} >= {"uq_provider_instrument_mappings_provider_listing", "uq_provider_instrument_mappings_provider_symbol"}
    assert {c.name for c in price.constraints if isinstance(c, UniqueConstraint)} >= {"uq_daily_prices_listing_date_type"}
...
def test_mapping_round_trip_preserves_domain_value() -> None:
    value = ProviderInstrumentMapping(id=uuid4(), workspace_id=uuid4(), listing_id=uuid4(), provider=MarketDataProvider.EODHD, provider_symbol="sie.xetra", provider_exchange_code="xetra", status=MappingStatus.ACTIVE, validated_at=NOW, validation_message=None, created_at=NOW, updated_at=NOW, version=1)
