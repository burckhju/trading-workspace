...
async def test_slow_discovery_does_not_block_warrant_quote_lane(monkeypatch) -> None:
    ...
    try:
        ...
        assert value.status()["lanes"][RefreshLane.WARRANT_DISCOVERY.value]["running"] is True
    finally:
...
async def test_leader_stops_both_workers_before_unlock_and_probes_during_active_jobs(
    ...
    active_lanes = (RefreshLane.WARRANTS, RefreshLane.UNDERLYINGS)
    started = {lane: asyncio.Event() for lane in active_lanes}
    stopped = {lane: asyncio.Event() for lane in active_lanes}
