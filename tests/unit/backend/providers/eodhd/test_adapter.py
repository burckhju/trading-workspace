...
class Mappings:
    ...
    async def get_mapping(self, workspace_id, mapping_id):
        return self.value


class Currencies:
    async def get_currency(self, workspace_id, listing_id):
        return "USD"
...
def make_adapter():
    ...
    adapter = EodhdMarketDataAdapter(
        ...
        rate_limiter=TokenBucketRateLimiter(
            requests_per_second=10, burst_capacity=2, clock=clock, sleeper=sleeper
        ),
...
async def test_latest_returns_newest_completed_row() -> None:
    ...
    result = await adapter.get_latest_completed_daily_price(
        LatestDailyPriceRequest(
            workspace_id=mapping.workspace_id,
            listing_id=mapping.listing_id,
            mapping_id=mapping.id,
